...
- The React/Vite frontend now implements the outcome-led institutional landing page,
  ...
  `createRootAccord` intent, then exposes explicit FXRP approval, collateral
  lock, and bounded `openSyndication` intents using the same borrower wallet.
  It stops at the coordinator boundary and does not reuse the recorded
  facility or claim a public coordinator runner. Its build and nineteen
  semantic, invariant, learning, and wayfinding tests pass locally. The
...
- The institutional frontend now builds canonical draw calldata locally,
  ...
  The Borrower Sandbox can also prepare the fresh-facility `createRootAccord`,
  FXRP `approve`, `lockCollateral`, and bounded `openSyndication` intents,
  where `msg.sender` becomes the borrower. It does not surface provider
  quotes, FCC/CoFill evidence, materialization, provider funding, verifier,
  draw, repayment, or automatic runner actions; those remain explicit
  team-operated boundaries.
