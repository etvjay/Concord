...
CONTRACT_FILE="ConcordInstructionSender.sol"
CONTRACT_NAME="ConcordInstructionSender"
GO_PKG="concordsender"
...
if ! grep -q "contract ${CONTRACT_NAME}" "$PROJECT_DIR/contracts/${CONTRACT_FILE}" 2>/dev/null; then
    echo ""
    echo "ERROR: Contract name '${CONTRACT_NAME}' not found in contracts/${CONTRACT_FILE}."
    echo "Make sure the contract name in ${CONTRACT_FILE} matches CONTRACT_NAME in this script."
    exit 1
fi

echo "=== Step 2: Extract ABI and BIN ==="
FORGE_OUT="$PROJECT_DIR/out/${CONTRACT_FILE}/${CONTRACT_NAME}.json"
if [[ ! -f "$FORGE_OUT" ]]; then
