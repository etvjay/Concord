#!/usr/bin/env bash
# test.sh — Send Concord instructions on-chain, poll for results, and validate correctness.
#
...
#   CONCORD_QUOTES      — comma-separated signed QuoteRequest JSON files
#   CONCORD_FINALIZE    — FinalizeRoundRequest JSON file
#   CONCORD_EVIDENCE_OUT — signed FCC evidence output path (default: evidence/fcc-action.json)
set -euo pipefail
...
CONCORD_QUOTES="${CONCORD_QUOTES:-}"
CONCORD_FINALIZE="${CONCORD_FINALIZE:-}"
CONCORD_EVIDENCE_OUT="${CONCORD_EVIDENCE_OUT:-$PROJECT_DIR/evidence/fcc-action.json}"

[[ -n "$INSTRUCTION_SENDER" ]] || die "INSTRUCTION_SENDER not set. Run pre-build.sh first or set it manually."
[[ -n "$CONCORD_QUOTES" ]] || die "CONCORD_QUOTES must contain comma-separated signed quote JSON files"
[[ -n "$CONCORD_FINALIZE" ]] || die "CONCORD_FINALIZE must point to a FinalizeRoundRequest JSON file"
...
resolve_file() {
    ...
    [[ -f "$value" ]] || die "$label file not found: $value"
    printf '%s/%s\n' "$(cd "$(dirname "$value")" && pwd)" "$(basename "$value")"
}

resolve_quote_files() {
    local raw="$1"
    local item resolved joined=""
    local -a items=()
    IFS=',' read -r -a items <<< "$raw"
    for item in "${items[@]}"; do
        item="${item#"${item%%[![:space:]]*}"}"
        item="${item%"${item##*[![:space:]]}"}"
        [[ -n "$item" ]] || die "CONCORD_QUOTES contains an empty path"
        resolved="$(resolve_file "$item" "quote")"
        if [[ -n "$joined" ]]; then
            joined+=","
        fi
        joined+="$resolved"
...
QUOTE_FILES="$(resolve_quote_files "$CONCORD_QUOTES")"
FINALIZE_FILE="$(resolve_file "$CONCORD_FINALIZE" "finalize")"
if [[ "$CONCORD_EVIDENCE_OUT" != /* ]]; then
    CONCORD_EVIDENCE_OUT="$PROJECT_DIR/$CONCORD_EVIDENCE_OUT"
fi
mkdir -p "$(dirname "$CONCORD_EVIDENCE_OUT")"
...
log "Quote files:        $QUOTE_FILES"
log "Finalize payload:   $FINALIZE_FILE"
log "FCC evidence:       $CONCORD_EVIDENCE_OUT"
